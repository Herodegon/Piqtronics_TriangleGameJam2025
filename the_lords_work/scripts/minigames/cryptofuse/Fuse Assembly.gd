...
func generate_slot(contacts: bool):
	...
	original_fuse_visibility = false
...
func set_fuse_type(c: Color, t: int):
	fuse_color = c
	fuse_type = t
	original_fuse = $Fuse.get_data()
	original_fuse_visibility = true

func reset_fuse():
	if original_fuse:
		set_fuse(original_fuse)
	set_fuse_visibility(original_fuse_visibility)
